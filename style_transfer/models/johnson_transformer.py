...
# Wrapper class that cli.py expects
class JohnsonNet(nn.Module):
    """A wrapper for the Johnson (Fast Neural Style) TransformerNetwork.
    This class is intended to be used by the main pipeline to provide a consistent interface.
    It holds an already initialized and potentially pre-trained TransformerNetwork instance.
    """
    def __init__(self, model: TransformerNetwork):
        super(JohnsonNet, self).__init__()
        if not isinstance(model, (TransformerNetwork, TransformerNetworkTanh)):
            raise ValueError("The 'model' argument must be an instance of TransformerNetwork or TransformerNetworkTanh.")
        self.model = model

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Passes the input through the wrapped Johnson model."""
        return self.model(x)